...
nodes_number = 20 #change accordingly to 5,10,20
transition_prob = 0.25 # change accordingly to 0.5,0.25,0.25
...
nx.draw(er_igraph, pos=pos, with_labels=True, edge_color='gray', node_size=800,node_color="orange", font_size=12)
...
number_samples = 8000000 # change accordingly 
((nodes_number**4)*np.log(nodes_number))/(transition_prob**2)
number_samples
number_samples> (nodes_number**4)*np.log(nodes_number)*(transition_prob**(-2))

bytes_needed = number_samples * nodes_number * 4   # if float32
print(bytes_needed / 1024**3, "GB needed")

mem = psutil.virtual_memory()
print(f"Total RAM: {mem.total / 1024**3:.2f} GB")
print(f"Available RAM: {mem.available / 1024**3:.2f} GB")

# Setting the configuration for the Erdos-Reyni graph
...
eta_parameters = np.arange(0.5, 20.5, 0.5)
...
for k, eta in enumerate(eta_parameters):
    ...
    mean = np.zeros(nodes_number, dtype=np.float32),
    cov = covariance_matrix.astype(np.float32),
    size = number_samples).astype(np.float32)

    y_samples = rng.multivariate_normal(
    mean=np.zeros(nodes_number, dtype=np.float32),
    cov=covariance_matrix_Y.astype(np.float32),
    size=number_samples).astype(np.float32)
...
heat_map_matrix.sort_values(by="Forbenius error")
